package miscellaneous;

public class Style {
  static final public String BUTTON_GREEN = "-fx-background-color: rgb(192,236,204);";
  static final public String BUTTON_RED = "-fx-background-color:rgb(246, 168, 166);";
  static final public String BUTTON_NORMAL = "-fx-background-color:#DDD;";

  static final public String LABEL_GREEN = "-fx-color: rgb(192,236,204);";
  static final public String LABEL_RED = "-fx-color: rgb(246, 168, 166);";
  static final public String LABEL_NORMAL = "-fx-color: #333;";

  static final public String LABEL_GREEN_FAT = LABEL_GREEN + "-fx-font-weight: bold;";
  static final public String LABEL_RED_FAT = LABEL_RED + "-fx-font-weight:bold;";
  static final public String LABEL_NORMAL_FAT = LABEL_NORMAL + "-fx-font-weight:bold;";

  static final public String TEXTFIELD_GREEN = "-fx-background-color:#FFF; -fx-border-color:rgb(192,236,204); -fx-border-radius: 5; -fx-text-fill:#333;";
  static final public String TEXTFIELD_RED = "-fx-background-color:#FFF; -fx-border-color:rgb(246, 168, 166); -fx-border-radius: 5; -fx-text-fill:#333;";
  static final public String TEXTFIELD_NORMAL = "-fx-background-color:#FFF; -fx-border-color:#DDD; -fx-border-radius: 5; -fx-text-fill:#333;";

  static final public String TOGGLESWITCH = "-fx-border-radius:15;";
  static final public String BACKGROUND_BLUE = "-fx-background-color:  rgb(165,200,228);";
  static final public String BACKGROUND_LIGHTGRAY = "-fx-background-color: #FAFAFA;";
  static final public String BACKGROUND_NONE = "-fx-background: none;";
  static final public String BACKGROUND_WHITE = "-fx-background: #FFF;";
}
